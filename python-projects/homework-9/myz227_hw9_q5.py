...
class InvertedFile:
    def __init__(self, file_name):
        self.word_table = ChainingHashTableMap()
        ...
        for i in range(len(self.words)):
            ...
            last_char = self.words[i][-1]
            if last_char in punct:
                self.words[i] = self.words[i].strip(last_char)
        for index in range(len(self.words)):
            try:
                old_table = self.word_table[self.words[index]]
                old_table.append(index)
                self.word_table[self.words[index]] = old_table
            except KeyError:
                self.word_table[self.words[index]] = [index]
