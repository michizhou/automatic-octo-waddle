...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if (requestLine[REQUEST_TAR] == NULL)
            {
                error(400);
                continue;
            }
            else if (requestLine[REQUEST_TAR][0] != '/')
            {
                error(501);
                continue;
            }
            else if (strchr(requestLine[REQUEST_TAR], '\"') != NULL)
            {
                error(400);
                continue;
            }
            
            if (requestLine[HTTP_VER] == NULL || strcmp ("HTTP/1.1", requestLine[HTTP_VER]) != 0)
            {
                error(505);
                continue;
            }
            ...
            char* queryI = strchrnul(requestLine[REQUEST_TAR], '?');
            char* eosI = strchr(requestLine[REQUEST_TAR], '\0');
            char query[(eosI - queryI < 1) ? 1: eosI - queryI];
            memcpy(
                query, 
                (strncmp(queryI, "\0", 1) == 0) ? queryI : queryI + 1, 
                (eosI - queryI < 1) ? 1 : eosI - queryI
            );
            
            queryI = strchrnul(requestLine[REQUEST_TAR], '?');
            char absPath[queryI - requestLine[REQUEST_TAR] + 1];
            strcpy(
                absPath,
                requestLine[REQUEST_TAR],
                queryI - requestLine[REQUEST_TAR]
            );
            absPath[queryI - requestLine[REQUEST_TAR]] = '\0';
            
            char* extI = strchr(absPath, '.');
            if (extI == NULL)
            {
                error(501);
                continue;
            ...
            eosI = strchr(absPath, '\0');
            char extension[eosI - extI];
            memcpy(extension, extI + 1, eosI - extI);
